# Fair experiment sweep: unique prompts so ngram cannot memorize identical replies.
...
$resultsPath = Join-Path $root "bench-fair-experiments.json"
...
function Start-Server([string[]]$extra) {
  ...
  $proc = Start-Process -FilePath $exe -ArgumentList $argList -PassThru -WindowStyle Hidden `
    -RedirectStandardOutput "$env:TEMP\malaikat-fair-out.txt" `
    -RedirectStandardError "$env:TEMP\malaikat-fair-err.txt"
  ...
  while ((Get-Date) -lt $deadline) {
    ...
    if ($proc.HasExited) {
      $err = Get-Content "$env:TEMP\malaikat-fair-err.txt" -Raw -ErrorAction SilentlyContinue
      $out = Get-Content "$env:TEMP\malaikat-fair-out.txt" -Raw -ErrorAction SilentlyContinue
...
foreach ($v in $variants) {
  ...
  try {
    ...
    $out = Join-Path $root ("bench-fair-{0}.json" -f $v.name)
    $env:MALAIKAT_BENCH_LABEL = $v.name
    $env:MALAIKAT_BENCH_RUNS = "5"
    $env:MALAIKAT_BENCH_N = "128"
    $env:MALAIKAT_BENCH_OUT = $out
    & powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "bench-fair.ps1")
    ...
    $all += $row
    Write-Host ("SUMMARY {0}: short_tg={1}±{2} long_tg={3}±{4} long_pp={5}" -f $row.name, $row.short_tg, $row.short_tg_stdev, $row.long_tg, $row.long_tg_stdev, $row.long_pp) -ForegroundColor Green
  } catch {
...
Write-Host "`n======== FAIR COMPARISON (unique prompts) ========" -ForegroundColor Yellow
$all | Sort-Object long_tg, short_tg -Descending | Format-Table name, short_tg, short_tg_stdev, short_pp, long_tg, long_tg_stdev, long_pp, ok -AutoSize
